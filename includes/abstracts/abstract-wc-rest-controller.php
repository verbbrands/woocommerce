 ...
 *
 * This is a copy of the WP_REST_Controller.
 * We can extend WP_REST_Controller class once it's included in the WP core.
 *
 * @author   WooThemes
 * @category API
	...
	public function validate_schema_property( $value, $request, $parameter ) {
		...
		if ( ! empty( $property['enum'] ) ) {
			if ( ! in_array( $value, $property['enum'] ) ) {
				return new WP_Error( 'rest_invalid_param', sprintf( __( '%s is not one of %s', 'woocommerce' ), $parameter, implode( ', ', $property['enum'] ) ) );
			}
		}

		if ( 'integer' === $property['type'] && ! is_numeric( $value ) ) {
			return new WP_Error( 'rest_invalid_param', sprintf( __( '%s is not of type %s', 'woocommerce' ), $parameter, 'integer' ) );
		}

		if ( 'string' === $property['type'] && ! is_string( $value ) ) {
			return new WP_Error( 'rest_invalid_param', sprintf( __( '%s is not of type %s', 'woocommerce' ), $parameter, 'string' ) );
		}
